...
# this document is the entire workflow for the tutorial
...
targets %>% 
  ...
  autoplot() + # uses ggplot to create time series when recognizing tsibble 
...
## ----get training data---------------------------------------------------------------------------------------------------------------------------
# pivot wider takes values from one column and creates new columns with it (reducing number of rows), and in our case uses the label in 'variable' column as the new column headers  
...
# Using a log(x + 1) transform on the abundance data, which is right skewed 
mod_fits <- targets_train %>% 
  tsibble::fill_gaps() %>% # gap fill the data for the random walk model - if a timestep is missing, fills value with NA for that time
  fabletools::model(
...
# TSLM linear model with time series components 
...
# look at fit stats and identify the best model using AIC, where the lowest AIC is the best model (Akaike Information Criterion) 
...
# neon4cast::efi_format takes a fable ts forecast and formats to be in EFI standard for submitting to challenge, which creates and ensemble 
